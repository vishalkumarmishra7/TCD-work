...
def multi_process(i, df, date):
        ...
        try:
            ...
            model = auto_arima(df2t, trace=False, error_action='ignore', suppress_warnings=True)
            model.fit(df2t)
...
def make_a_move(df3,dfl,path,num_investments):
    ...
    df3 = df3[df3['diff'] > 0].sort_values('AIC').iloc[:num_investments,]
    df3['Investment'] = min(balance/num_investments,1000/num_investments)
    
    # df3['Investment'] = np.where(df3['diff'] > 0.01, balance/num_investments, 0)
...
src = "AI Stock Project/"

data_prep(path = src + "Stock codes.csv", dest = src + "main_data.csv", distinct_stocks = 400)

df = read_stock_data(path = src + "main_data.csv")
l = [['Date','Balance']]
date = '2020-01-01'
num_investments = 5
...
while date < df['Date'].max():
    ...
    dfl_csv.to_csv(src + 'Historical Ledger.csv')
